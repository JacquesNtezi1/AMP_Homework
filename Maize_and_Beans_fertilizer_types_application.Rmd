---
title: 'Maize and Beans fertilizer types application '
...
output:
  ...
  word_document:
    toc: yes
    toc_depth: '6'
subtitle: Maize and Beans Fertiizer types application report
---
...
# Study Background
N/A ( no information provided , the same as trial methodology )

# Trial Objective
Determine which type  of fertilizer leads to the highest Yield  for Maize and/or Beans in the Bugarama region . 
 
# Trial Hypothesis 
The fertilizer Type B  will produce significantly higher yields than fertilizer type A 
...
pivot<- ddply(c,.(Group),summarize,count=length(Group),
                                   aveg_maize_yield=mean(Maize.yield.per.acre),
                                   aveg_maize_yield=mean(Beans.yield.per.acre))

```

# Results Summary Report 
 
MAIZE AND BEANS YIELD RESPONSE TO FERTILIZER TYPES ( A and B) IN BUGARAMA REGION 

Field experiment was carried out at Bugarama region  to evaluate the effect of two types of fertilizer on Yield of Maize  Beans  . Maize and Beans were grown with fertilzer grade A and B.All the treatments A and B have the same number of farmer ,500 for each treatment and the yield were measured . 
